...
export default new class Function {
    ...
    async getFile(PATH, save = false) {
        try {
            ...
            if (data && save) {
                filename = path.join(__dirname, '../tmp/', `${name}_BOTBE-${this.getRandom("", "3")}.` + type.ext)
 ...
 */
async getFiles(PATH, saveToFile = false, name = "") {
    const directory = "./tmp";
    if (!fs.existsSync(directory)) fs.mkdirSync(directory)
    let filename, data;
    if (/^https?:\/\//.test(PATH)) {
        let response = await fetch(this.isUrl(PATH))
    ...
    let size = Buffer.byteLength(data)
    if (data && saveToFile && !filename) (filename = path.join(__dirname, '../tmp/'+ `${name}_BOTBE-${this.getRandom("", "5")}.`+ type.ext), await fs.promises.writeFile(filename, data))
    return {
        filename, 
        ...type, 
        data, 
