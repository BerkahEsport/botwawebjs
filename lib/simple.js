...
const {Message, MessageMedia, Contact, Location, Buttons, List } = mywajs
...
async reply(jid, teks, quoted) {
  let mentions = await conn.parseMention(teks) || []
  let m;
  try {
...
async sendAd(jid, link = '', caption = '', quoted) {
        conn.sendMessage(jid , link, {
                ...
                mentions: quoted?.isGroup ? quoted.mentions : conn.parseMention(caption) || [],
                extra: {
                    ctwaContext: {
