...
=> *ɴᴀᴍᴀ ᴀɴᴅᴀ:* ${ conn.getName( m.sender ) }
...
${wm}`.trim()
	...
	if ( user.registered === true )
		throw m.reply(`[💬] Kamu sudah terdaftar\nMau daftar ulang? *${ usedPrefix }unreg [SERIAL NUMBER]*`)
	if ( !Reg.test( text ) )
		return conn.reply( m.chat, belumdaftar, m )
	let [ _, name, splitter, age ] = text.match( Reg )
	if ( !name )
		throw m.reply('Nama tidak boleh kosong (Alphanumeric)')
	if ( !age )
		throw m.reply('Umur tidak boleh kosong (Angka)')
	age = parseInt( age )
	if ( age > 30 )
		throw m.reply('WOI TUA (。-`ω´-) \n\nᴜᴅᴀʜ ᴛᴜᴀ ʙᴀɴʏᴀᴋɪɴ ɪʙᴀᴅᴀʜ ᴊᴀɴɢᴀɴ ᴍᴀɪɴᴀɴ ᴍᴜʟᴜ!!!')
	if ( age < 5 )
		throw m.reply('Halah dasar bocil')
